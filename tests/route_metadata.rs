use dx_route_transitions::{NavigationAnimation, RouteTransitions, route_transitions};
...
#[test]
fn browser_back_uses_route_layer_semantics_not_the_fallback_pair() {
    let pushed = Route::Person { id: 7 };
    let cover = Route::Editor {};
    let misleading_fallback = Route::Detail { id: 99 };

    // Detail and Person are forward destinations from each other, so a pair
    // alone is ambiguous. A real history traversal is nevertheless a pop.
    assert_eq!(
        pushed.transition_to(&Route::Detail { id: 7 }),
        NavigationAnimation::PushLeft
    );
    assert_eq!(
        pushed.transition_back(&misleading_fallback),
        NavigationAnimation::PushRight
    );
    assert_eq!(
        cover.transition_back(&misleading_fallback),
